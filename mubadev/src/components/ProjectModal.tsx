...
export const ProjectModal: React.FC<ProjectModalProps> = ({ project, onClose }) => {
  ...
  return (
    <AnimatePresence>
      {project && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 sm:p-6 font-mono">
          {/* Backdrop Escuro */}
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          <motion.div
            ...
            className="relative w-full max-w-5xl bg-panel border border-border rounded-xl shadow-2xl overflow-hidden z-10 flex flex-col max-h-[90vh]"
          >
            ...
            <div className="flex items-center justify-between px-4 py-3 bg-panel-2 border-b border-border-soft shrink-0">
              ...
              <button
                onClick={onClose}
                className="text-text-faint hover:text-orange-1 text-sm font-bold px-2 py-0.5 rounded border border-transparent hover:border-border transition-all"
              >
                [ESC / X]
              </button>
            </div>
            ...
            <div className="p-5 sm:p-6 grid grid-cols-1 lg:grid-cols-2 gap-6 overflow-y-auto">
              ...
              <div className="flex flex-col gap-3">
                ...
                <div className="relative w-full aspect-video rounded-lg overflow-hidden border border-border bg-black">
                  {project.youtubeId ? (
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center text-text-faint gap-2 p-4 text-center">
              ...
              <div className="flex flex-col justify-between gap-4 font-mono">
                <div>
                  ...
                  {/* Descrição Longa */}
                  <div className="text-[13px] text-text-dim leading-relaxed mb-5 bg-panel-2 border border-border-soft p-3.5 rounded-lg">
                ...
                <div className="pt-4 border-t border-border flex flex-wrap gap-3">
                  {project.githubUrl && (
                    <a
                      ...
                      className="flex-1 text-[12px] px-4 py-2.5 rounded bg-panel-2 border border-border text-text font-bold text-center transition-all duration-150 hover:border-orange-1 hover:text-orange-1 flex items-center justify-center gap-2"
                    >
                  ...
                  {project.deployUrl && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex-1 text-[12px] px-4 py-2.5 rounded bg-orange-1 text-black font-bold text-center transition-all duration-150 hover:bg-orange-2 flex items-center justify-center gap-2"
                    >
                      <span>./ver_site</span>
                    </a>
                  )}
